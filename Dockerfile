...
# Operational config, not app code (see tenancy/registry.py's docstring for
# why it lives at repo-root config/ rather than packaged inside src/):
# build_default_tenant_registry() resolves config/tenants relative to the
# process CWD, which is this WORKDIR at container runtime.
