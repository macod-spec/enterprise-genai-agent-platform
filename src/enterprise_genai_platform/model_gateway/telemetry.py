...
def record_success(span: Span, result: ModelGenerationResult, *, tenant: str) -> None:
    ...
    MODEL_TOKENS.labels(tenant, result.provider, "input").inc(result.usage.prompt_tokens)
    MODEL_TOKENS.labels(tenant, result.provider, "output").inc(result.usage.completion_tokens)
    MODEL_ESTIMATED_COST_GBP.labels(tenant, result.provider).inc(result.estimated_cost_gbp)
    MODEL_GATEWAY_CALLS.labels(tenant, result.model, result.provider, "success").inc()
    MODEL_GATEWAY_DURATION.labels(tenant, result.model, result.provider).observe(
...
def record_failure(
    ...
    *,
    ...
    MODEL_GATEWAY_CALLS.labels(tenant, model, provider, outcome).inc()
    MODEL_GATEWAY_DURATION.labels(tenant, model, provider).observe(duration_seconds)
