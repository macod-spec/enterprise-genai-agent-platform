...
async def evaluate_cases(cases: tuple[EvaluationCase, ...]) -> EvaluationReport:
    ...
    for case in cases:
        result = await workflow.investigate(
            ...
            # Mirrors gateway/app.py's union of MCP tool-invocation
            # capability with tenant document-retrieval entitlements: the
            # golden set evaluates agent behaviour across every synthetic
            # policy domain, not one tenant's scoped subset.
