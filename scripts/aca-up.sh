...
my_public_ip="${MY_PUBLIC_IP:?Set MY_PUBLIC_IP (run: curl ifconfig.me) so ingress can be locked to it}"
...
# (docs/portfolio/live-verification.md, gateway/auth.py). This is a real,
# documented platform limitation, not something this script works around —
# the IP restriction below is the actual access control for this deployment.
...
az containerapp ingress access-restriction set \
  ...
  --rule-name my-ip --ip-address "${my_public_ip}/32" --action Allow
