...
the `customer` entitlement never sees it — not merely ranked lower, genuinely
absent — even though it would be returned first for a fully-entitled caller.
A test that only checked the happy path (entitled caller sees it) would
prove nothing about security trimming. `caller_roles` here is a tenant
bundle's `entitlements` (config/tenants/*.yaml), matching every other
retrieval call site post multi-tenancy.
...
_CUSTOMER_DATA_QUERY = (
    ...
    "the customer entitlement. Suspected disclosure must be escalated for human "
    "review."
)
...
def test_entitled_caller_retrieves_the_document() -> None:
    ...
    hits = asyncio.run(
        index.search(
            _CUSTOMER_DATA_QUERY,
            query_embedding,
            caller_roles=frozenset({"customer"}),
            limit=5,
        )
    )
...
def test_unentitled_caller_never_receives_the_top_ranked_document() -> None:
    ...
    absent for a caller missing the required entitlement — not merely re-ranked
    lower. `complaints` mirrors the complaints-triage tenant bundle, which
    legitimately has no `customer` entitlement."""
    embedding = LocalHashEmbedding()
    index = _index()
    query_embedding = embedding.embed(_CUSTOMER_DATA_QUERY)
    ...
    unentitled_hits = asyncio.run(
        index.search(
            _CUSTOMER_DATA_QUERY,
            query_embedding,
            caller_roles=frozenset({"complaints"}),
            limit=5,
        )
    )
    ...
    assert "POL-DATA-003" not in returned_document_ids, (
        "entitlement leak: caller without the customer entitlement retrieved a "
