...
def test_role_filter_requires_all_chunk_roles_to_be_held_by_caller() -> None:
    ...
    full_access = build_role_filter(frozenset({"transactions", "customer", "complaints"}))
    partial_access = build_role_filter(frozenset({"transactions", "customer"}))

    assert full_access == "allowed_roles/all(r: not search.in(r, '', '|'))"
    assert partial_access == "allowed_roles/all(r: not search.in(r, 'complaints', '|'))"
...
def test_role_filter_denies_by_default_when_caller_has_no_roles() -> None:
    ...
    assert (
        empty_filter
        == "allowed_roles/all(r: not search.in(r, 'complaints|customer|transactions', '|'))"
    )
