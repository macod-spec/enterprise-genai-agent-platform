...
def caller(agent: AgentIdentity) -> CallerContext:
    # roles mirrors production's gateway/app.py, which unions the caller's
    # MCP tool-invocation capability (agent.invoke) with its tenant's
    # document-retrieval entitlements. This helper stands in for a caller
    # fully entitled across every synthetic policy domain, since these
    # tests exercise agent capability, not tenant entitlement scoping.
    return CallerContext(
        subject="test-user",
        roles=frozenset({"agent.invoke", "transactions", "customer", "complaints"}),
        agent=agent,
