...
def test_skill_api_requires_platform_viewer_role() -> None:
    ...
    # payment-disputes' bundle allows 2 of the 3 registered skills; the
    # response reflects tenant entitlement scoping, not the full registry.
    assert {skill["name"] for skill in allowed.json()["skills"]} == {
        "payment-investigation",
        "customer-profile",
    }
