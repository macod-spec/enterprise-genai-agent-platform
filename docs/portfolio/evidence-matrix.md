...
| Multi-tenancy | `VERIFIED-LOCAL` | Tenant config bundles (`config/tenants/*.yaml`), Postgres RLS with `FORCE ROW LEVEL SECURITY` and transaction-scoped `set_config`, tenant-scoped Redis/SQLite state, entitlement-filtered retrieval, per-tenant token budgets, tenant-labelled metrics, CI zero-code-name-drift gate (`scripts/check-tenant-drift.sh`) | `tests/test_tenancy_leakage.py` (six-point cross-tenant leakage suite, written and confirmed failing before the implementation existed), `tests/integration/test_postgres_rls_pool.py` (real Postgres, pool size 1 forcing physical connection reuse across tenants — this test caught a real RLS bypass during development: the fixture initially connected as the Postgres superuser, which ignores `FORCE ROW LEVEL SECURITY` unconditionally) | A 4th tenant (`fraud-alerts`) was added as a single YAML file with zero `src/` changes and confirmed working end-to-end (skills, retrieval, workflow) — verifying the zero-code onboarding claim directly, not via the Backstage agent/skill scaffolder above, which onboards a different kind of thing (a new agent capability, not a tenant) and has no tenant-specific template. `SupervisorWorkflow`'s pre-tenant-resolution routing-preview metric is intentionally left labelled `unscoped` (`docs/adrs/015-multi-tenancy-isolation.md`) rather than expanded into agent-internal state |
...
| Live demo endpoint | `VERIFIED-LIVE` | Azure Container Apps deployment of the real signed image (Task 3), real Azure OpenAI/AI Search/Content Safety adapters, gateway auth + per-tenant token budget as access control (no IP allowlist, 2026-08-13) | `docs/portfolio/live-verification.md`, `scripts/aca-up.sh`/`aca-down.sh` | Interim path while AKS is quota-blocked; local-identity auth only (no verified production identity provider); demo-only, torn down between sessions |
