...
from Task 3, system-assigned identity with real RBAC (no keys). Access
control is the gateway's own auth (every agent endpoint returns 401
without valid `X-Local-*` headers) plus a low per-tenant token budget, not
an IP allowlist — a rotating home IP is unsuitable for that role and was
deliberately not used (2026-08-13). `scripts/aca-up.sh` /
`scripts/aca-down.sh` capture the deployment as a reusable, idempotent
script rather than one-off commands. Full narrative and honest boundaries
in `docs/portfolio/limitations.md` — in particular, `APP_ENV=local` (the
