...
**Portal click path:** Azure Portal → Subscriptions → (select the target
subscription — `az account show --query id --output tsv`, or
`${AZURE_SUBSCRIPTION_ID}` if scripting) → **Usage + quotas** → filter
Provider = `Compute`, Location = `UK South` → search `Dnsv6` → select
`standardDnv6Family` → **Request quota increase** → enter new limit →
submit.

| Field | Value |
| --- | --- |
| Subscription | `${AZURE_SUBSCRIPTION_ID}` (run `az account show --query id --output tsv` to resolve) |
| Region | UK South |
| Provider | Compute |
